...
def predict(data, data_numeric, trained_dir, output='features'):
    ...
    numeric_columns = list(set(data_numeric.columns) - set(preprocess_config['result']) - {'vid'})
    ...
    input_x3 = np.array(data_numeric, dtype=np.float32)
    ...
    batches = load_data.batch_iter(list(zip(input_x1, input_x2, input_x1_ratio, input_x2_ratio, input_x3)), params['batch_size'], num_epochs=1, shuffle=False)
    batch_count = 0
    output_data = []
    ...
    for batch in batches:
        x_1_batch, x_2_batch, x_1_ratio_batch, x_2_ratio_batch, x_3_batch = zip(*batch)
        output_data_batch = predict_function(x_1_batch, x_2_batch, x_1_ratio_batch, x_2_ratio_batch, x_3_batch)
...
def write_features(data_path, data_numeric_path, trained_dir):
    ...
    columns_sequences_count_dict1, columns_sequences_count_dict2 = predict(data, data_numeric, trained_dir)
...
def write_result(data_path, data_numeric_path, trained_dir, test=False):
    data = pickle.load(open(data_path, 'rb'))
    data_numeric = pickle.load(open(data_numeric_path), 'rb')
    ...
    output_result, _, preprocess_config, _, _, _, _ = predict(data, data_numeric, trained_dir, output='result')
