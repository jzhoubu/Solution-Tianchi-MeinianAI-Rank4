...
from data_helper.load_data_v3 import load_data
from data_helper.load_data import batch_iter
from model.model_v3_double_cnn_join_numeric.double_multi_cnn_join_numeric_model import DoubleMultiCnnJoinNumericModel
from model.model_v3_double_cnn_join_numeric.predict import predict_by_model
...
tf.app.flags.DEFINE_integer('decay_steps', 1500, 'how many steps before decay learning_rate.')
...
tf.app.flags.DEFINE_integer('num_epochs', 120, 'num of epochs to run.')
...
tf.app.flags.DEFINE_integer('hidden_size1', 24, 'hidden size of output dense layer')
...
tf.app.flags.DEFINE_float('l2_lambda', 0.00002, 'l2_regularization')
...
tf.app.flags.DEFINE_float('learning_rate_threshold', 0.0003, 'slow down the decay of lr if lr lower than threshold')
...
def train(data_path, data_numeric_path, pretrain_embedding_path=None, pretrain_model_path=None):
    # 生成训练数据
    data_x_1, data_x_2, data_x_1_ratio, data_x_2_ratio, data_x_3, data_y, word_embedding, vocabulary, vocabulary_inv,\
    ...
    data_x_3, data_x_3_test, \
    data_y, data_y_test = train_test_split(
        data_x_1, data_x_2, data_x_1_ratio, data_x_2_ratio, data_x_3, data_y, test_size=0.1)
    ...
    data_x_3_train, data_x_3_dev, \
    data_y_train, data_y_dev = train_test_split(
        data_x_1, data_x_2, data_x_1_ratio, data_x_2_ratio, data_x_3, data_y, test_size=0.1)

    logging.info('data_x_1: {}, data_x_2: {}, data_x_3: {}'.format(len(data_x_1_train), len(data_x_2_train), len(data_x_3_train)))
    ...
    params['numeric_size'] = len(numeric_columns)
    ...
    # 初始化Model
    model = DoubleMultiCnnJoinNumericModel(
        # batch_size
    ...
    train_batches = batch_iter(list(zip(
        data_x_1_train, data_x_2_train, data_x_1_ratio_train, data_x_2_ratio_train,
        data_x_3_train, data_y_train)), FLAGS.batch_size, FLAGS.num_epochs)
    ...
    while True:
        ...
        x_1_train_batch, x_2_train_batch, x_1_ratio_train_batch, x_2_ratio_train_batch, x_3_train_batch, y_train_batch = zip(*train_batch)
        train_loss = model.train_step(x_1_train_batch, x_2_train_batch, x_1_ratio_train_batch, x_2_ratio_train_batch, x_3_train_batch, y_train_batch, FLAGS.dropout_keep_prob)
        ...
        if current_step % FLAGS.validate_step == 0:
            ...
            dev_batches = batch_iter(list(zip(data_x_1_dev, data_x_2_dev, data_x_1_ratio_dev, data_x_2_ratio_dev, data_x_3_dev, data_y_dev)), FLAGS.batch_size, 1)
            dev_loss, dev_batch_num = 0.0, 0
            for dev_batch in dev_batches:
                dev_batch_num += 1
                x_1_dev_batch, x_2_dev_batch, x_1_ratio_dev_batch, x_2_ratio_dev_batch, x_3_dev_batch, y_dev_batch = zip(*dev_batch)
                dev_loss_batch = model.dev_step(x_1_dev_batch, x_2_dev_batch, x_1_ratio_dev_batch, x_2_ratio_dev_batch, x_3_dev_batch, y_dev_batch)
    ...
    test_batches = batch_iter(list(zip(data_x_1_test, data_x_2_test, data_x_1_ratio_test, data_x_2_ratio_test, data_x_3_test, data_y_test)), FLAGS.batch_size, 1)
    test_loss, test_batch_num = 0.0, 0
    for test_batch in test_batches:
        test_batch_num += 1
        x_1_test_batch, x_2_test_batch, x_1_ratio_test_batch, x_2_ratio_test_batch, x_3_test_batch, y_test_batch = zip(*test_batch)
        test_loss_batch = model.dev_step(x_1_test_batch, x_2_test_batch, x_1_ratio_test_batch, x_2_ratio_test_batch, x_3_test_batch, y_test_batch)
